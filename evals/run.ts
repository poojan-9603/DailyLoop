...
try {
  execSync("npx tsx --env-file=.env.local evals/smart-log.eval.ts", { stdio: "inherit" });
} catch {
...
console.log("─".repeat(40));
try {
  execSync("npx tsx --env-file=.env.local evals/plan-generator.eval.ts", { stdio: "inherit" });
} catch {
